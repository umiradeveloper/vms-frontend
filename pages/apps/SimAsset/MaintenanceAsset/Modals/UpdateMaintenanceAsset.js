...
const UpdateMaintenanceAsset = ({ openModal, setOpenModal, loader, setLoader, reload, setReload }) => {
    ...
    const [dataMaintenance, setDataMaintenance] = useState({
     ...
     const getKondisi = async() => {
        ...
        try {
            const result = await api.get(apiUrl + "/asset-manajemen/get-maintenance-kondisi", {
                headers: {
                    "Content-Type": "application/json",
                }
            });
     ...
     const getStatus = async() => {
        ...
        try {
            const result = await api.get(apiUrl + "/asset-manajemen/get-maintenance-status", {
                headers: {
                    "Content-Type": "application/json",
                }
            });
     ...
     const create = async() => {
            ...
            setLoader(true);
            var fm = new FormData();
            fm.append("id_maintenance_asset", dataMaintenance.id_maintenance_asset);
            ...
            fm.append("status_maintenance", dataMaintenance.status_maintenance);
            fm.append("tanggal_selesai", dataMaintenance.tanggal_selesai);
            fm.append("kondisi_setelah", dataMaintenance.kondisi_setelah);
            
             try {
                const result = await api.post(apiUrl + "/asset-manajemen/update-maintenance-asset", fm, {
                    headers: {
    ...
    useEffect(() => {
        ...
        if(openModal.open){
            setDataMaintenance({
                id_maintenance_asset: openModal.datas?.id_maintenance_asset,
                id_asset: openModal.datas?.asset?.id_asset,
                tipe_maintenance: openModal.datas?.tipe_maintenance,
                biaya: openModal.datas?.biaya,
                deskripsi_maintenance: openModal.datas?.deskripsi,
                kondisi_setelah: openModal.datas?.kondisi_setelah,
                status_maintenance: openModal.datas?.status_maintenance,
                tanggal_maintenance: openModal.datas?.tanggal_maintenance,
                tanggal_selesai: openModal.datas?.tanggal_selesai
            })
                        ...
                        <div className="row gy-2 pb-3">
                            ...
                            <Col xl={6} >
                                <Form.Group controlId="formFile" className="mb-3">
                                    ...
                                    <Select name="state" className="basic-multi-select" options={asset} isSearchable value={asset.find(
                                        x => x.value === dataMaintenance.id_asset
                                    )}
                            ...
                            <Col xl={6} >
                                <Form.Group controlId="formFile" className="mb-3">
                                    ...
                                    <Select name="state" className="basic-multi-select" options={tipeMaintenance} isSearchable value={tipeMaintenance.find(
                                        x => x.value === dataMaintenance.tipe_maintenance
                                    )}
                            ...
                            <Col xl={6} >
                                <Form.Group controlId="formFile" className="mb-3">
                                    ...
                                    <Form.Control type="text" placeholder="Biaya" value={dataMaintenance.biaya} onChange={(e) => setDataMaintenance({...dataMaintenance, biaya: e.target.value})} />
                                </Form.Group>
                            </Col>
                            ...
                            <Col xl={12} >
                                <Form.Group controlId="formFile" className="mb-3">
                                    ...
                                    <Form.Control as="textarea" id="text-area" rows={2} value={dataMaintenance.deskripsi_maintenance} onChange={(e) => setDataMaintenance({...dataMaintenance, deskripsi_maintenance: e.target.value})} />
                                </Form.Group>
                            </Col>
                            <Col xl={6} >
                                <Form.Group controlId="formFile" className="mb-3">
                                    <Form.Label>Status Maintenance <span style={{ color: "red" }}>*</span> </Form.Label>
                                    <Select name="state" className="basic-multi-select" options={statusMaintenance} isSearchable value={statusMaintenance.find(
                                        x => x.value === dataMaintenance.status_maintenance
                                    )}
                                        menuPlacement='auto' classNamePrefix="Select2" placeholder="Pilih Status Maintenance" onChange={(e) => setDataMaintenance({...dataMaintenance, status_maintenance: e.value})}
                                    />
                                </Form.Group>
                            </Col>
                            <Col xl={6}>
                                <Form.Group controlId="formFile" className="mb-3">
                                    <Form.Label>Tanggal Tanggal Selesai <span style={{ color: "red" }}>*</span> </Form.Label>
                                        <Flatpickr
                                            className="form-control"
                                            value={dataMaintenance.tanggal_selesai ?? ""}
                                            options={{
                                                dateFormat: "Y-m-d",
                                                enableTime: false,
                                                time_24hr: false,
                                            }}
                                            onChange={(val, valStr) =>  setDataMaintenance({ ...dataMaintenance, tanggal_selesai: valStr })}
                                            // onChange={(val,valStr) => {
                                            // 	console.log(valStr)
                                            // }}
                                            placeholder="Tanggal Maintenance"
                            ...
                            <Col xl={12} >
                                <Form.Group controlId="formFile" className="mb-3">
                                    <Form.Label>Kondisi Setelah <span style={{ color: "red" }}>*</span> </Form.Label>
                                    <Select name="state" className="basic-multi-select" options={kondisiSetelah} isSearchable value={kondisiSetelah.find(
                                        x => x.value === dataMaintenance.kondisi_setelah
                                    )}
                                        menuPlacement='auto' classNamePrefix="Select2" placeholder="Pilih Kondisi Setelah" onChange={(e) => setDataMaintenance({...dataMaintenance, kondisi_setelah: e.value})}
                                    />
                                </Form.Group>
                            </Col>
