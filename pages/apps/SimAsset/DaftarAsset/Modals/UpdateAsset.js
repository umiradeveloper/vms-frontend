...
const UpdateAsset = ({openModal, setOpenModal, reload, setReload, loader, setLoader}) => {
    ...
    const create = async() => {
        ...
        setLoader(true);
        var fm = new FormData();
        fm.append("id_asset", dataAsset.id_asset);
        ...
        fm.append("umur_ekonomis", dataAsset.umur_ekonomis);
         try {
            const result = await api.post(apiUrl + "/asset-manajemen/update-asset", fm, {
                headers: {
    ...
    const setData = async(datas) => {
        setDataAsset({...datas, id_user: datas.user_pemilik?.id_user, foto: null});
    }
    ...
    return(
        <Modal size="lg" show={openModal.open} onHide={() => setOpenModal({ ...openModal, open: false })}>
            <Modal.Header>
                <h6 className="modal-title" id="exampleModalLabel">Update Asset</h6>
            </Modal.Header>
            <Modal.Body>
                <Row>
                    ...
                    <Col xl={12} className="rounded-3">
                        <div className="row gy-2 pb-3">
                            ...
                            {/* <Col xl={6} >
                             ...
                             <Col xl={6}>
                                <div className="row gy-2 pb-3">
                                    <label htmlFor="nama-proyek" className="form-label ">Penanggung Jawab<span style={{ color: "red" }}>*</span> :</label>
                                    <Select name="state" className="basic-multi-select " options={user} isSearchable value={user.find(
                                        x => x.value === dataAsset.id_user
                                    )}
                            ...
                            <Col xl={6}>
                                <div className="row gy-2 pb-3">
                                    <label htmlFor="nama-proyek" className="form-label ">Kondisi<span style={{ color: "red" }}>*</span> :</label>
                                    <Select name="state" className="basic-multi-select " options={kondisi} isSearchable value={kondisi.find(
                                        x => x.value === dataAsset.kondisi
                                    )}
                            ...
                            <Col xl={6}>
                                <div className="row gy-2 pb-3">
                                    <label htmlFor="nama-proyek" className="form-label ">Status Asset <span style={{ color: "red" }}>*</span> :</label>
                                    <Select name="state" className="basic-multi-select " options={statusAsset} isSearchable value={statusAsset.find(
                                        x => x.value === dataAsset.status_asset
                                    )}
