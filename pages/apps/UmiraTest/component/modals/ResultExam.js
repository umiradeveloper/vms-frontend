...
const ResultExam = ({ openModal, setOpenModal }) => {
    const [datatable, setDatatable] = useState([]);
    const [exam, setExam] = useState({});
    const COLUMNS = [
        {
            Header: "Tanggal Submit",
            accessor: "tanggal_submit"
        },
        {
            Header: "Nama",
            accessor: "nama"
        ...
        },
        {
            ...
            accessor: "wrong_answer"
        },
        {
            Header: "Tidak Di Jawab",
            accessor: "un_answer"
        },
        {
            Header: "Status",
            accessor: "status_pass"
        },
    ...
    useEffect(() => {
        if (openModal.open) {
            ...
            if (openModal.datas?.examResult?.length > 0) {
                for (const r of openModal.datas?.examResult) {
                    arrResult.push({
                        tanggal_submit: formatDate(r.date_submit_exam),
                        nama: r.user?.nama,
                        role: r.user?.role?.nama_role,
                        score: r.score,
                        correct_answer: r.correct_answer,
                        wrong_answer: r.wrong_answer,
                        un_answer: r.un_answer,
                        status_pass: r.status_pass
                    })
                }
            }
            setDatatable(arrResult);
    ...
    return (
        <Modal size="xl" show={openModal.open} onHide={() => { setOpenModal({ ...openModal, open: false }) }} enforceFocus={false}>
            <Modal.Header>
                <h6 className="modal-title" id="exampleModalLabel">Result Test</h6>
            </Modal.Header>
            <Modal.Body>
                <Row>
                    <Col xl={12}>
...
export default dynamic(() => Promise.resolve(ResultExam), { ssr: false });