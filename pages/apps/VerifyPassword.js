...
const VerifyPassword = () => {
    ...
    const getVerifiedToken = async () => {
        ...
        setLoader(true);
        try {
            const response = await axios.get(apiUrl+"/auth/verify-token?token="+params.get("token"), {
                headers:{
    ...
    const ResetPassword = async () => {
        ...
        setLoader(true);
        try {
            const response = await axios.post(apiUrl+"/auth/reset-password",{token: params.get("token"), password: password},{
                headers:{
                    "Content-Type":"application/json"
                }
            });
												...
												<div className="main-container container-fluid">
													<Row className="row-sm">
														<Card.Body className="mt-2 mb-2">
															...
															<div className="clearfix"></div>
                                                            {expired ? (
                                                                <h2 className="text-start mb-2">EXPIRED</h2>
                                                            ):(
                                                                <form>
                                                                    <h5 className="text-start mb-2">Reset Password</h5>
...
export default dynamic(() => Promise.resolve(VerifyPassword), { ssr: false });
