...
const ApprovalAttendance = ({loader, setLoader}) => {
    ...
    const getApprovalAbsensi = async () => {
        const apiUrl = process.env.NEXT_PUBLIC_API_URL;
        setLoader(true);
        try {
            const result = await apiConfig.get(apiUrl + "/HR-Attendance//get-approval-attendance", {
                headers: {
                    ...
                    for (const datas of result.data.data) {
                        attendanceArr.push({
                            ...
                            aksi: <div className="d-flex flex-row gap-2">
                                <button className="btn btn-success" onClick={() => { HandleApprove(datas.id_pengajuan_absensi) }} >Approve</button>
                                <button className="btn btn-danger" onClick={() => { HandleReject(datas.id_pengajuan_absensi) }} >Reject</button>
                            </div>
                        })
                    }
    ...
    const HandleApprove = async(id) => {
        ...
        if (resultConfirm.status) {
            ...
            try {
                const result = await apiConfig.get(apiUrl + "/HR-Attendance/approval-attendance?id_pengajuan_absensi=" + id+"&status_approval=Approve", {
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": "Bearer " + localStorage.getItem("token")
                    }
    ...
    const HandleReject = async(id) => {
        ...
        if (resultConfirm.status) {
            ...
            try {
                const result = await apiConfig.get(apiUrl + "/HR-Attendance/approval-attendance?id_pengajuan_absensi=" + id+"&status_approval=Reject&catatan="+resultConfirm.value, {
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": "Bearer " + localStorage.getItem("token")
                    }
