...
const DetailApprovalPengajuan = ({ openModal, setOpenModal, loader, setLoader }) => {
    ...
    const HandleNotVerified = async (id) => {
        const resultConfirm = await AlertConfirm("Apakah anda yakin ingin tidak memverifikasi data ini ? ", "warning", "Not Verified", false, "Data berhasil Confirm");
    ...
    const SubmitData = async () => {
        ...
        fm.append("status_approval", updatePengajuanApproval.status_approval);
    ...
    return (
        <Modal size="xl" show={openModal.open} onHide={() => { setOpenModal({ ...openModal, open: false }) }}>
            ...
            <Modal.Footer className="d-flex gap-2">
                {showData.status_approval && showData.layak_bayar && showData.bukti_bayar ? (
                    <></>
                ) : (
                    <Button variant='contained' type="button" className="btn btn-primary"
                        data-bs-dismiss="modal" onClick={SubmitData}>Submit</Button>
                )}
